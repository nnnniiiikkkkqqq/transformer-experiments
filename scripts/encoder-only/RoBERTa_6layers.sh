#!/bin/bash
#SBATCH --job-name=RoBERTa_6layers       # Название эксперимента
#SBATCH --output=../../logs/encoder-only/RoBERTa_6layers/output.log  # Путь к логу вывода
#SBATCH --error=../../logs/encoder-only/RoBERTa_6layers/error.log    # Путь к логу ошибок
...
python RoBERTa_6layers.py

# Пример: Сохранение результатов
# cp model.pth ../../results/encoder-only/RoBERTa_6layers/