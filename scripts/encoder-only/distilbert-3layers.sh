#!/bin/bash
#SBATCH --job-name=distilbert-3layers       # Название эксперимента
#SBATCH --output=../../logs/encoder-only/distilbert-3layers/output.log  # Путь к логу вывода
#SBATCH --error=../../logs/encoder-only/distilbert-3layers/error.log    # Путь к логу ошибок
...
python distilbert-3layers.py

# Пример: Сохранение результатов
# cp model.pth ../../results/encoder-only/distilbert-3layers/