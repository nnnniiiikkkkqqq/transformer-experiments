#!/bin/bash
#SBATCH --job-name=bert_base_fillmask       # Название эксперимента
#SBATCH --output=../../logs/encoder-only/bert_base_fillmask/output.log  # Путь к логу вывода
#SBATCH --error=../../logs/encoder-only/bert_base_fillmask/error.log    # Путь к логу ошибок
...
python bert_base_fillmask.py

# Пример: Сохранение результатов
# cp model.pth ../../results/encoder-only/bert_base_fillmask/