#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Experiment: BERT with 384 Embedding Size (Initialized from Scratch) on IMDb and Yelp Polarity for Binary Sentiment Classification
...
config = BertConfig(
    vocab_size=30522,  # Same as bert-base-uncased
...
training_args_imdb = TrainingArguments(
    output_dir="./../../results/results_bert_384_embedding_imdb",
    num_train_epochs=num_epochs_imdb,
    per_device_train_batch_size=batch_size,
    per_device_eval_batch_size=batch_size,
    evaluation_strategy="epoch",
    save_strategy="epoch",
    logging_dir="./logs_bert_384_embedding_imdb",
    logging_steps=100,
...
print(f"|-----------------------|----------|----------|---------------------|-------------|-------------------|")
print(f"| BERT-384-embedding    | {accuracy_imdb:.4f}   | {f1_imdb:.4f}   | {inference_time_imdb:.2f}               | {memory_used:.2f}         | {training_time_imdb:.2f}         |")
...
training_args_yelp = TrainingArguments(
    output_dir="./../../results/results_bert_384_embedding_yelp",
    num_train_epochs=num_epochs_yelp,
    per_device_train_batch_size=batch_size,
    per_device_eval_batch_size=batch_size,
    evaluation_strategy="epoch",
    save_strategy="epoch",
    logging_dir="./logs_bert_384_embedding_yelp",
    logging_steps=100,
    fp16=True,
...
print(f"|-------------------------------|-----------------|-----------------|---------------------|-------------|----------------------|")
print(f"| BERT-384-embedding (fine-tuned) | {accuracy_yelp:.4f}          | {f1_yelp:.4f}          | {inference_time_yelp:.2f}          | {memory_used_yelp:.2f}       | {training_time_yelp:.2f}         |")