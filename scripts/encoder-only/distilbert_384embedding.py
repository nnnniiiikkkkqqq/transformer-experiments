#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Experiment: DistilBERT with 384 Embedding Size (Initialized from Scratch) on IMDb and Yelp Polarity for Binary Sentiment Classification

import time
import torch
from transformers import DistilBertForSequenceClassification, DistilBertTokenizer, Trainer, TrainingArguments, DistilBertConfig
...
# Create a custom configuration with reduced embedding size (hidden_size=384)
config = DistilBertConfig(
    vocab_size=30522,  # Same as distilbert-base-uncased
    hidden_size=384,  # Reduced from 768 to 384
    num_hidden_layers=6,  # DistilBERT has 6 layers
    num_attention_heads=12,
    intermediate_size=1536,  # 4 * hidden_size for feed-forward layer
    max_position_embeddings=512,
    num_labels=2,
)

# Load tokenizer and initialize model with custom configuration (from scratch)
...
training_args_imdb = TrainingArguments(
    output_dir="./../../results/results_distilbert_384_embedding_imdb",
    num_train_epochs=num_epochs_imdb,
    per_device_train_batch_size=batch_size,
    per_device_eval_batch_size=batch_size,
    evaluation_strategy="epoch",
    save_strategy="epoch",
    logging_dir="./logs_distilbert_384_embedding_imdb",
    logging_steps=100,
...
print(f"|---------------------------|----------|----------|---------------------|-------------|-------------------|")
print(f"| DistilBERT-384-embedding  | {accuracy_imdb:.4f}   | {f1_imdb:.4f}   | {inference_time_imdb:.2f}               | {memory_used:.2f}         | {training_time_imdb:.2f}         |")
...
training_args_yelp = TrainingArguments(
    output_dir="./../../results/results_distilbert_384_embedding_yelp",
    num_train_epochs=num_epochs_yelp,
    per_device_train_batch_size=batch_size,
    per_device_eval_batch_size=batch_size,
    evaluation_strategy="epoch",
    save_strategy="epoch",
    logging_dir="./logs_distilbert_384_embedding_yelp",
    logging_steps=100,
    fp16=True,
...
print ("Results table for Yelp fine-tuning evaluation:")
print ("| Model                             | Accuracy (Yelp) | F1-score (Yelp) | Inference Time (ms) | Memory (GB) | Fine-tuning Time (s) |")
print ("|-----------------------------------|-----------------|-----------------|---------------------|-------------|----------------------|")
print(f"| DistilBERT-384-embedding (fine-tuned) | {accuracy_yelp:.4f}          | {f1_yelp:.4f}          | {inference_time_yelp:.2f}          | {memory_used_yelp:.2f}       | {training_time_yelp:.2f}         |")