...
config = BertConfig.from_pretrained(model_name, num_attention_heads=num_attention_heads, num_labels=2)
...
training_args = TrainingArguments(
    output_dir="./../../results/results_bert_6heads",
    num_train_epochs=num_epochs,
    per_device_train_batch_size=batch_size,
    per_device_eval_batch_size=batch_size,
    evaluation_strategy="epoch",
    save_strategy="epoch",
    logging_dir="./logs_bert_6heads",
    logging_steps=100,
...
print(f"|----------------------|----------|----------|---------------------|-------------|-------------------|")
print(f"| BERT-6-heads         | {accuracy:.4f}   | {f1:.4f}   | {inference_time:.2f}               | {memory_used:.2f}         | {training_time:.2f}         |")
...
training_args_yelp = TrainingArguments(
    output_dir="./results_bert_6heads_yelp",
    num_train_epochs=1,  # Fewer epochs for fine-tuning
    per_device_train_batch_size=batch_size,
    per_device_eval_batch_size=batch_size,
    evaluation_strategy="epoch",
    save_strategy="epoch",
    logging_dir="./logs_bert_6heads_yelp",
    logging_steps=100,
    fp16=True,
...
print(f"|---------------------------|-----------------|-----------------|---------------------|-------------|----------------------|")
print(f"| BERT-6-heads (fine-tuned) | {accuracy_yelp:.4f}          | {f1_yelp:.4f}          | {inference_time_yelp:.2f}          | {memory_used:.2f}       | {training_time_yelp:.2f}         |")